...
import cn.net.sinodata.cm.pb.LogosFileTransfer.EResultStatus;
import cn.net.sinodata.cm.pb.LogosFileTransfer.MsgBatchInfo;
import cn.net.sinodata.cm.pb.LogosFileTransfer.MsgFileInfo;
...
public class UpdateBatchService extends HttpServlet{
...
//			if (items != null) {
//				//转换成批次对象，可直接用于持久化
//				batchInfo = parseItem(fileContents, items);
//				logger.info("开始处理添加批次请求，批次号：" + batchInfo.getBatchId());
//				//调用管理服务添加批次
//				manageService.addBatch(batchInfo, fileContents);
//				result = "{'result': '1'}";
//			}else{	//失败
//				result = "{'result': '0', 'errMsg': '解析请求失败，请求中不包含任何对象'}";
//			}
		} catch (Exception e) {
			result = "{'result': '0', 'errMsg': " + "'上传文件失败, " + e.getMessage() + "'}";
			logger.error(e);
		} finally {
	...
	private void processFromUpload(FileItem item, HttpServletResponse hsr)
	{
		...
		try	{
			...
			if (fname.endsWith(OpeMetaFileUtils.PBOPEEXT)) {
				...
				if (batchs.containsKey(mfileinfo.getBatchId())){
					BatchInfo batchInfo = batchs.get(mfileinfo.getBatchId());
				...
				// 上传不合规的数据，直接
				result.setStatus(EResultStatus.Failed);
				result.setMsg("上传数据内容的扩展名非" + OpeMetaFileUtils.PBDataExt + "或者" + OpeMetaFileUtils.PBOPEEXT + "服务拒绝");
			}
		}
		catch (Exception e){
			logger.debug("添加批次失败，", e);
			result.setStatus(EResultStatus.Failed);
			result.setMsg(e.toString());
		}
		try {			
			result.toNetMsg().writeTo(hsr.getOutputStream());
		}
		catch (Exception e){
			e.printStackTrace();
		}
